...
This library depends on the [`jjwt`][jjwt] libraries.
...
- `uk.gov.dbt.ndtp.servlet.auth.jwt.jaxrs3.JwtAuthFilter` for JAX-RS 3 i.e. `jakarta.ws.rs` applications.
- `uk.gov.dbt.ndtp.servlet.auth.jwt.servlet3.JwtAuthFilter` for Servlet 3 i.e. `javax.servlet` applications.
- `uk.gov.dbt.ndtp.servlet.auth.jwt.servlet5.JwtAuthFilter` for Servlet 5 i.e. `jakarta.servlet` applications.
...
- `uk.gov.dbt.ndtp.servlet.auth.jwt.engine` - Specifies a `JwtAuthenticationEngine` implementation to use. If not specified
  then an appropriate default implementation for your runtime environment is used.
- `uk.gov.dbt.ndtp.servlet.auth.jwt.verifier` - Specifies a `JwtVerifier` implementation to use, **MUST** be provided or
...
<filter>
    ...
    <filter-class>uk.gov.dbt.ndtp.servlet.auth.jwt.servlet3.JwtAuthFilter</filter-class>
    <init-param>
...
You can configure exclusions by setting the `uk.gov.dbt.ndtp.servlet.auth.jwt.path-exclusions` Servlet Context attribute,
...
unique paths, then you will see warnings more frequently as the cache entries are evicted by new exclusion warnings
...
All of these derive from the same base class in `jwt-servlet-auth-lib` and share the following default behaviours:
